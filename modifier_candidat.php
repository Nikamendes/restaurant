...
try {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $sql = "UPDATE restaurant_candidats 
                SET email = :email, Nom = :nom, Prenom = :prenom, Experience = :experience, Adresse = :adresse, Disponibilite = :disponibilite, CV = :cv, mot_de_passe = :mot_de_passe, Annonce = :annonce
...
<body>
    ...
    <?php
    if (isset($candidat)) {
        // Le formulaire est rempli avec les données du candidat pour pouvoir les modifier
    ?>
    <form action="" method="POST">
        <input type="hidden" name="id_candidat" value="<?php echo $id_candidat; ?>">
        <div>
            <label for="email">Email :</label>
            <input type="text" id="email" name="email" required value="<?php echo $candidat['email']; ?>">
        </div>
        <div>
