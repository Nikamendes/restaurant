...
try {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        // Vérifier si l'id_annonce existe déjà dans la table
        $sql_check = "SELECT id_annonce FROM restaurant_annonces WHERE id_annonce = :id_annonce";
        $stmt_check = $conn->prepare($sql_check);
        $stmt_check->bindParam(':id_annonce', $id_annonce, PDO::PARAM_INT);
        $stmt_check->execute();

        if ($stmt_check->rowCount() > 0) {
            echo "Erreur : L'id_annonce existe déjà. Veuillez choisir un nouvel identifiant.";
        } else {
            // Requête préparée pour ajouter une nouvelle annonce dans la table 'restaurant_annonces'
            $sql = "INSERT INTO restaurant_annonces (id_annonce, annonce, lieu, description) VALUES (:id_annonce, :annonce, :lieu, :description)";
